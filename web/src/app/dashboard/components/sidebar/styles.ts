...
export const sidebarStyles = tv({
  slots: {
    container:
      "bg-white border-r border-slate-200 flex-shrink-0 transition-all duration-300 flex flex-col fixed left-0 top-0 h-screen z-30",
    logoContainer: "h-20 flex items-center px-5 border-b border-slate-100",
    logoIcon:
      "w-12 h-12 bg-teal-500 rounded-2xl flex items-center justify-center shadow-md",
    ...
    collapseButton:
      "absolute top-24 -right-3 w-6 h-6 bg-white border border-slate-200 rounded-full flex items-center justify-center hover:bg-slate-50 transition-colors shadow-sm z-10",
  },
  variants: {
    open: {
      true: { container: "w-64" },
