...
import type { ScheduleProps, Schedule as ScheduleType } from './types'
      ...
      <Dialog open={showEditDialog} onOpenChange={setShowEditDialog}>
        <DialogContent className="max-w-lg">
          ...
          <div className="space-y-3 mt-4 max-h-[60vh] overflow-y-auto">
            {schedules.map((schedule) => (
              <div key={schedule.id} className="flex items-center gap-3 p-3 bg-slate-50 rounded-lg">
                <div className="w-28">
                  <p className="text-sm font-medium text-slate-700">{schedule.label}</p>
                </div>
                ...
                {!schedule.isClosed && (
                  <div className="flex items-center gap-2">
                    <Input 
                      type="time"
                      value={schedule.open}
                      onChange={(e) => updateTime(schedule.day, 'open', e.target.value)}
                    ...
                    <Input 
                      ...
                      onChange={(e) => updateTime(schedule.day, 'close', e.target.value)}
                      className="w-24 text-sm"
                    />
                  </div>
                )}
