...
import { stockItems, stats as initialStats } from './data'
import { greeting } from '../dashboard/utils/greeting'
import type { StockItem, FilterOptions } from './types'
...
export default function EstoquePage() {
  ...
  const updateStats = (newItems: StockItem[]) => {
    setStats({
      ...
      lowStock: newItems.filter(i => i.quantity <= i.minQuantity && i.quantity > 0).length,
  ...
  const handleSave = (itemData: Omit<StockItem, 'id' | 'clinicId' | 'lastUpdated'>) => {
    let newItems: StockItem[]
    
    if (selectedItem) {
      newItems = items.map(i => i.id === selectedItem.id 
        ? { ...i, ...itemData, lastUpdated: new Date().toISOString().split('T')[0] } 
        : i
      )
    } else {
      const newId = Math.max(...items.map(i => i.id), 0) + 1
      newItems = [...items, { ...itemData, id: newId, clinicId: 1, lastUpdated: new Date().toISOString().split('T')[0] }]
    }
