...
import { getStockStatus, stockTypeLabels, stockTypeColors } from '../../types'
    ...
    <div className={s.container()} onClick={() => onClick?.(item)}>
      <div className={s.imageContainer()}>
        <img 
          ...
          className={s.image()}
          onError={(e) => {
            (e.target as HTMLImageElement).src = 'https://images.unsplash.com/photo-1584308666744-24a5c6f2302f?w=200&h=200&fit=crop'
          }}
        />
        <span className={`${s.typeBadge()} ${stockTypeColors[item.type as StockType]}`}>
      ...
      <div className={s.content()}>
        <div className={s.header()}>
          ...
          <span className={`${s.badge()} ${statusStyles[status.color].bg} ${statusStyles[status.color].text}`}>
            {status.label}
          </span>
        </div>
